function testRobustSort() {
  console.log('Testing robust sort with mixed data types (Plain JS)...');
  ...
  try {
    mockData.sort((a, b) => getMs(b.created_at) - getMs(a.created_at));
    console.log('Sort successful! Result order:');
    mockData.forEach(d => console.log(` - ${d.id}: ${JSON.stringify(d.created_at === undefined ? 'undefined' : d.created_at)}`));
