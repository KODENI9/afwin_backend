...
function resolveWinningNumber(snapshotTotals: Record<number, number>): number {
  const numbers = Object.keys(snapshotTotals).map(Number).filter(n => n >= 1 && n <= 9);
  const totals = numbers.map(n => snapshotTotals[n] || 0);
  ...
  const positiveEntries = numbers
    .map(n => ({ number: n, total: snapshotTotals[n] || 0 }))
    .filter(e => e.total > 0);
...
const tests = [
  ...
  },
  {
    ...
    input: { 1: 1000, 2: 200, 3: 300, 4: 4000, 5: 0, 6: 0, 7: 500, 8: 600, 9: 700 },
    expected: 2
  },
  {
  ...
  {
    name: "CAS 7 - Nombres invalides (ex: 100)",
    input: { 1: 1000, 2: 2000, 100: 10, 200: 5 },
    expected: 1
  }
];
